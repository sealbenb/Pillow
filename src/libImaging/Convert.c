...
#else
        static char buf[100];
        nsprintf(buf, 100, "conversion from %.10s to %.10s not supported", imIn->mode, mode);
                          ...
                          int r, int g, int b)
{
    ...
    {
      static char buf[100];
      nsprintf(buf, 100, "conversion from %.10s to %.10s not supported in convert_transparent", imIn->mode, mode);
      return (Imaging) ImagingError_ValueError(buf);
    }
#endif
